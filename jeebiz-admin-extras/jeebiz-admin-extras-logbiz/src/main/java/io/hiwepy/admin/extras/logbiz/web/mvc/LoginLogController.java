 ...
 * All Rights Reserved. 
 */
package io.hiwepy.admin.extras.logbiz.web.mvc;
...
import io.hiwepy.admin.extras.logbiz.utils.enums.AuthzOptEnum;
import io.hiwepy.admin.extras.logbiz.web.dto.AuthzLogDTO;
import io.hiwepy.admin.extras.logbiz.web.dto.AuthzLogPaginationDTO;
...
public class LoginLogController extends BaseApiController {
	...
	@Autowired
	private ILoginLogService authzLogService;
	...
	public ApiRestResponse<Result<AuthzLogDTO>> list(@Valid @RequestBody AuthzLogPaginationDTO paginationDTO)
		...
		LoginLogModel model = getBeanMapper().map(paginationDTO, LoginLogModel.class);
		Page<LoginLogModel> pageResult = getAuthzLogService().getPagedList(model);
		List<AuthzLogDTO> retList = new ArrayList<AuthzLogDTO>();
		for (LoginLogModel logModel : pageResult.getRecords()) {
	...
	public ILoginLogService getAuthzLogService() {
		return authzLogService;
	}

	public void setAuthzLogService(ILoginLogService authzLogService) {
