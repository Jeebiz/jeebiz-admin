...
public abstract class AbstractFilestoreStrategy implements FilestoreStrategy, InitializingBean, ApplicationEventPublisherAware {
	...
	protected <O extends FileUploadBO> boolean preCheck(O uploadBo) throws Exception {
		...
		if (getRedisOperation().sHasKey(blacklistKey, uploadBo.getUserId())) { 
			throw new BizRuntimeException(ApiCode.SC_FORBIDDEN, "sms.send.backlist.limit");
		}
		return true;
	}
	...
	protected abstract <O extends FileUploadBO> List<FileDTO> handleUpload(O uploadBo) throws Exception;
	
	protected <O extends FileUploadBO> void onUploadSuccess(O uploadBo, FileUploadResult uploadRt) throws Exception{
		
		log.debug("onUploadSuccess");
		
	};
	
	protected <O extends FileUploadBO> void onUploadComplete(O uploadBo, FileUploadResult uploadRt) throws Exception{
