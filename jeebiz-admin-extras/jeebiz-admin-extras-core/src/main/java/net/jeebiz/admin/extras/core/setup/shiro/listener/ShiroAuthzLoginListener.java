 ...
 * All Rights Reserved. 
 */
package net.jeebiz.admin.extras.core.setup.shiro.listener;
...
public class ShiroAuthzLoginListener implements LoginListener {
	...
	public void onSuccess(AuthenticationToken token, Subject subject, ServletRequest request,
		...
		Log4jUtils.instance("Authz-Log").info(Constants.authzMarker, String.format("账户:%s登录成功.", principal.getUsername()));
	...
	public void onFailure(AuthenticationToken token, Exception ex, ServletRequest request,
		...
		Log4jUtils.instance("Authz-Log").error(Constants.authzMarker, String.format("账户:%s登录失败.", userid), ExceptionUtils.getRootCause(ex));
