...
public class ShiroAuthzLogoutListener implements LogoutListener {
	...
	public void onFailure(Subject subject, Exception ex) {
		...
		Log4jUtils.instance("Authz-Log").error(Constants.authzMarker, String.format("用户:%s账户注销成功.", principal.getUsername()));
	...
	public void onSuccess(Subject subject, ServletRequest request, ServletResponse response) {
		...
		Log4jUtils.instance("Authz-Log").error(Constants.authzMarker, String.format("用户:%s账户注销失败.", principal.getUsername()));
