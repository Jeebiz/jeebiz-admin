...
CREATE TABLE `sys_article_category` (
  ...
  `c_status` tinyint(2) NOT NULL DEFAULT 1 COMMENT '文章分类状态（0:禁用|1:可用）',
...
CREATE TABLE `sys_article_topic` (
  ...
  `t_status` tinyint(2) NOT NULL DEFAULT 1 COMMENT '文章栏目状态（0:禁用|1:可用）',
...
CREATE TABLE `sys_article_contents` (
  ...
  `c_org_id` bigint(12) NOT NULL COMMENT '文章所属单位id/编号',
  `c_title` varchar(500) NOT NULL COMMENT '文章标题',
  `c_keywords` varchar(255) DEFAULT NULL COMMENT '文章关键字',
  `c_digest` text NOT NULL COMMENT '文章摘要',
  `c_content` text NOT NULL COMMENT '文章内容',
  `c_review` tinyint(1) DEFAULT 0 COMMENT '文章审核状态（0:未提交|1:待审核|2:审核通过|2:审核不通过）',
  `c_status` tinyint(1) DEFAULT 1 COMMENT '文章状态（0:禁用|1:可用）',
  `c_order` int(3) DEFAULT 0 COMMENT '文章排序',
  `c_recommend` tinyint(1) DEFAULT 0 COMMENT '文章是否推荐（0:未推荐|1:推荐）',
  `c_browse` bigint(12) DEFAULT 0 COMMENT '文章浏览数',
  `c_collect` bigint(12) DEFAULT 0 COMMENT '文章收藏数',
  `c_liked` bigint(12) DEFAULT 0 COMMENT '文章点赞数',
  `c_target` tinyint(1) DEFAULT 1 COMMENT '文章发布对象（1：全部，2：指定学院，3：指定个人）',
  ...
  `modify_time` timestamp DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`c_id`) USING BTREE,
   UNIQUE KEY (`c_tid`,`c_cid`,`c_org_id`,`c_uid`)
...
CREATE TABLE `sys_article_content_targets` (
  `t_id` bigint(12) NOT NULL AUTO_INCREMENT COMMENT '文章发布对象关联id',
...
CREATE TABLE `sys_article_content_atts` (
  ...
  `a_type` tinyint(1) DEFAULT 0 COMMENT '文章附件类型（1：标题图片，2：内容图片，3：文件普通附件）',
...
CREATE TABLE `sys_article_content_comments` (
  ...
  `c_review` tinyint(1) DEFAULT 0 COMMENT '文章评论审核状态（0:未通过|1:通过）',
  `c_status` tinyint(1) DEFAULT 1 COMMENT '文章评论状态（0:删除|1:正常）',
  `c_recommend` tinyint(1) DEFAULT 0 COMMENT '文章评论推荐（0:未推荐|1:推荐）',
...
CREATE TABLE `sys_article_messages` (
  ...
  `m_status` tinyint(1) DEFAULT 0 COMMENT '文章消息状态（0:未读|1:已读）',
...
CREATE TABLE `sys_article_visits` (
  ...
  `v_type` tinyint(1) DEFAULT 0 COMMENT '文章访问类型（0:点击|1:收藏|2:点赞）',
