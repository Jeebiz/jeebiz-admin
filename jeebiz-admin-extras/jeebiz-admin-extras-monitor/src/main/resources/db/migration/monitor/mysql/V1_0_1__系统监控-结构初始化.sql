...
CREATE TABLE `$${table-prefix}usage_cpu` (
  ...
  `cpu_total` double(16) NOT NULL COMMENT 'CPU总的使用率',
  `cpu_sys` double(16) NOT NULL COMMENT 'CPU系统使用率',
  `cpu_user` double(16) NOT NULL COMMENT 'CPU用户使用率',
  `cpu_wait` double(16) NOT NULL COMMENT 'CPU当前等待率',
  `cpu_free` double(16) NOT NULL COMMENT 'CPU当前空闲率',
...
CREATE TABLE `$${table-prefix}usage_mem` (
  `id` bigint(12) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `instance_id` varchar(100) NOT NULL COMMENT '服务器实例ID',
  `mem_total` double(16) NOT NULL COMMENT '内存总量',
  `mem_used` double(16) NOT NULL COMMENT '已用内存',
  `mem_free` double(16) NOT NULL COMMENT '剩余内存',
  `mem_usage` double(16) NOT NULL COMMENT '内存使用率',
  `create_time` timestamp DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `idx_time` (`create_time`)
...
CREATE TABLE `$${table-prefix}usage_disk` (
  ...
  `disk_total` double(16) NOT NULL COMMENT '磁盘总大小',
  `disk_used` double(16) NOT NULL COMMENT '已用磁盘大小',
  `disk_free` double(16) NOT NULL COMMENT '剩余磁盘大小',
  `disk_usage` double(16) NOT NULL COMMENT '磁盘使用率',
  `create_time` timestamp DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `idx_time` (`create_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='服务器内存使用记录表';
