...
platform.Stop=function(success,failx){
	...
	var transform=function(){
	    alert("开始转换格式");
	    alert("dwxData.list:"+dwxData.list);
	...
	var stopFn=function(){
		...
		alert("结束录音共"+upIds.length+"段，开始上传下载");
		...
		var down=function(){
			...
			config.DownWxMedia({
				...
				if(/amr/i.test(data.mime)){
				  ...
				  alert("微信服务器返回了未知音频类型："+data.mime);
		...
		var up=function(){
			...
			alert("微信录音片段"+upidx+" wx.playVoice({localId:'"+localId+"'})");
	...
	if(WXRecordData.err){
	   alert("录制失败，已录制"+timeouts.length+"分钟，但后面出错："+WXRecordData.err);
